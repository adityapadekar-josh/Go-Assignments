...
func updateWebsiteStatus(ctx context.Context, wg *sync.WaitGroup, statusChecker StatusChecker, website string, dataStore *models.DataStore) {
	defer wg.Done()

	timeoutCtx, cancel := context.WithTimeout(ctx, 5*time.Second)
	defer cancel()

	status, err := statusChecker.Check(timeoutCtx, website)
...
func StatusCheckerCronJob(ctx context.Context, statusChecker StatusChecker, dataStore *models.DataStore) {
	...
	for website := range dataStore.Data {
		wg.Add(1)
		go updateWebsiteStatus(ctx, &wg, statusChecker, website, dataStore)
	}
